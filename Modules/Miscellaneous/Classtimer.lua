...
if (C and C.Medias and C.Medias.Font) then
	...
	MASTER_FONT = {C.Medias.Font, 12, "OUTLINE"};

	-- Sets font for stack count
	STACKS_FONT = {C.Medias.Font, 11, "OUTLINE"};
else
	-- Sets font for all texts
...
local TRINKET_FILTER = {
		CreateSpellEntry(236373), -- Norgannon's Foresight
		CreateSpellEntry(214404), -- Magtheridon's Might
	};
...
local CLASS_FILTERS = {
		...
		MAGE = {
			target = { 
				CreateSpellEntry(228358), -- Winter's Chill
			},
			player = {
				CreateSpellEntry(11426), -- Ice Barrier
				CreateSpellEntry(205473), -- Icicles
				CreateSpellEntry(12472), -- Icy Veins
				CreateSpellEntry(195418), -- Chain Reaction (Ebonchill)
				--CreateSpellEntry(195391), -- Jouster
				

			},
			procs = {
				CreateSpellEntry(190446), -- Brain Freeze
...
do
		...
		CreateAuraBar = function(parent)
			...
			if (bit.band(ICON_POSITION, 4) == 0) then		
				...
				if (bit.band(ICON_POSITION, 2) == 2) then
					...
					icon:SetPoint(iconAnchor1, result, iconAnchor2, iconOffset * (-BAR_HEIGHT - 1), 3);
				end			
				icon:Size(BAR_HEIGHT + 6);
			...
			bar:SetStatusBarTexture([=[Interface\Addons\RobsonUI\Core\Textures\Skullflower]=]);
	...
	CreateAuraBarFrame = function( dataSource, parent )
		...
		background:SetTexture([=[Interface\Addons\RobsonUI\Core\Textures\Skullflower]=]);
